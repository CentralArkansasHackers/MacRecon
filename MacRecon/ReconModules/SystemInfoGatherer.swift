...
import CoreGraphics
import AppKit  // Add this import for NSUserName

// Ensure the StealthUtils class is in scope
// If needed, you might need to make it public or use a module import
...
class SystemInfoGatherer {
    ...
    private func getAllUsers() -> [UserInfo] {
        ...
        while let userPtr = getpwent() {
            ...
            if user.pw_gid == adminGroupID {
                ...
                var groups = [Int32](repeating: 0, count: 32) // Changed to Int32
                var count: Int32 = 32
                if getgrouplist(user.pw_name, Int32(user.pw_gid), &groups, &count) >= 0 { // Type conversion
                    for i in 0..<Int(count) {
                        if Int(groups[i]) == Int(adminGroupID) { // Type conversion
                            isAdmin = true
                            break
                        }
                    }
    ...
    private func checkIfUserIsAdmin() -> Bool {
        ...
        if getgrouplist(username, Int32(getgid()), &groups, &groupCount) != -1 { // Type conversion
            // Get admin GID
            if let adminGroup = getgrnam("admin") {
                ...
                for i in 0..<Int(groupCount) {
                    if Int(groups[i]) == Int(adminGID) { // Type conversion
                        isAdmin = true
                        break
                    }
                }
    ...
    private func checkSIPStatus() -> Bool {
        ...
        // Check System Integrity Protection status
            ...
            private func getMACAddress(for interface: String) -> String? {
                ...
                let result = IOServiceGetMatchingServices(kIOMainPortDefault, IOServiceMatching("IONetworkInterface"), &iterator)
            ...
            private func checkRemoteLoginEnabled() -> Bool {
                ...
                if FileManager.default.fileExists(atPath: sshdPath) {
                    let processes = StealthUtils.getRunningProcesses()
                    return processes.contains { $0.contains("sshd") }
                }
